...
void Population::resize(std::int64_t n) {
    ...
    vivo.resize(N); es_delincuente.resize(N); meses_enfermo.resize(N); recibe_ayuda.resize(N); desplazado.resize(N); genero_diverso.resize(N);
...
std::int64_t Population::nacer(Sexo s, std::int32_t hogar, std::uint16_t municipio, std::uint8_t depto) {
    ...
    vivo.push_back(1); es_delincuente.push_back(0); meses_enfermo.push_back(0); recibe_ayuda.push_back(0); desplazado.push_back(0); genero_diverso.push_back(0);
...
Population build_synthetic(std::int64_t n, std::uint64_t seed) {
    ...
    for (std::int64_t i = 0; i < n; ++i) {
        ...
        // género diverso ~1.3% (15+), DETERMINISTA por índice para no perturbar el stream RNG
        p.genero_diverso[i] = (edad >= 15 && (static_cast<std::uint64_t>(i) * 2654435761ULL >> 8) % 1000 < 13) ? 1 : 0;
